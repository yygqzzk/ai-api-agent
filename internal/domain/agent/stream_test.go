...
// mockLLMClientForStream is a simple mock for stream testing
type mockLLMClientForStream struct{
	shouldError bool
}

func (m *mockLLMClientForStream) Next(_ context.Context, _ []Message, _ []ToolDefinition) (LLMReply, error) {
	if m.shouldError {
		return LLMReply{}, fmt.Errorf("mock LLM error")
	}
	return LLMReply{
...
func TestRunStreamEmitsEvents(t *testing.T) {
	...
	engine := NewAgentEngine(&mockLLMClientForStream{}, dispatcher, WithMaxSteps(5))
...
func TestRunStreamErrorEvent(t *testing.T) {
	...
	mockLLM := &mockLLMClientForStream{shouldError: true}
	engine := NewAgentEngine(mockLLM, dispatcher, WithMaxSteps(5))
