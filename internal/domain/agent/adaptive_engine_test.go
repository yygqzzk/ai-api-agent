...
func (s *stubTaskDispatcher) Dispatch(_ context.Context, name string, args json.RawMessage) (any, error) {
	...
	case "search_api":
		query := payload["query"]
		return tool.SearchAPIResult{Items: []tool.SearchAPIItem{{Endpoint: endpointForQuery(query), Snippet: query}}}, nil
	case "analyze_dependencies":
		return tool.AnalyzeDependenciesResult{Endpoint: payload["endpoint"], Dependencies: []string{"接口依赖A", "接口依赖B"}}, nil
	default:
