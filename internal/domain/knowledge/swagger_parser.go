...
func ParseSwaggerFile(path string, service string) ([]model.Endpoint, error) {
...
func ParseSwaggerBytes(body []byte, service string) ([]model.Endpoint, error) {
...
func ParseSwaggerDocumentBytes(body []byte, service string) (ParsedSpec, error) {
	...
	for _, path := range paths {
		...
		for _, method := range methods {
			...
			ep.Responses = make([]model.Response, 0, len(responseCodes))
			for _, code := range responseCodes {
				ep.Responses = append(ep.Responses, model.Response{
					StatusCode:  code,
