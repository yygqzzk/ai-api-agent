...
func (c *OpenAICompatibleLLMClient) Next(ctx context.Context, messages []Message, tools []ToolDefinition) (LLMReply, error) {
	...
	for attempt := 0; attempt <= c.maxRetries; attempt++ {
		...
		case <-ctx.Done():
			timer.Stop()
			return LLMReply{}, fmt.Errorf("context canceled while waiting retry: %w", ctx.Err())
		case <-timer.C:
		}
	}
	return LLMReply{}, lastErr
...
func (c *OpenAICompatibleLLMClient) doRequest(ctx context.Context, payload []byte) (LLMReply, time.Duration, bool, error) {
	...
	if err != nil {
		if ctx.Err() != nil {
			return LLMReply{}, 0, false, fmt.Errorf("send chat completion request: %w", err)
		}
		return LLMReply{}, 0, true, fmt.Errorf("send chat completion request: %w", err)
	}
	...
	if resp.StatusCode >= 300 {
		...
		return LLMReply{}, retryAfter, retryable, fmt.Errorf("chat completion API error %d: %s", resp.StatusCode, string(body))
	}
	...
	if len(parsed.Choices) == 0 {
		return LLMReply{}, 0, false, fmt.Errorf("chat completion response has no choices")
	}

	msg := parsed.Choices[0].Message
	reply := LLMReply{Content: strings.TrimSpace(msg.Content)}
